...
module.exports = function(focusState, fs, focusPatterns, directory, files, webpackCallback) {
  const focusedFiles = []
  const directories = []
  ...
  function repeater(index) {
    if (index >= files.length) {
      ...
      const result = focusState.enabled ? directories.concat(focusedFiles) : files
